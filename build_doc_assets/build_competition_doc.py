...
def make_custom_figures():
    ...
    create_diagram(
        ...
        "Edgi Talk M55 小智/呼噜/SOS 板系统架构",
        {
            ...
            "imu": {"xy": (70, 410, 320, 570), "title": "IMU", "desc": "跌落 / 打翻\n无法呼救场景触发", "fill": "#FDEFF4"},
...
def build_doc():
    ...
    add_para(doc, "随着居家养老、慢病管理与睡眠健康关注度持续提升，夜间睡眠阶段的呼吸异常、跌落风险、环境不舒适和无法主动求助等问题逐渐成为家庭看护的薄弱环节。传统摄像头看护存在隐私压力，可穿戴设备又容易因佩戴不适、忘记充电或摘除而中断监测。针对上述问题，本作品提出并实现了一套多模态非接触式睡眠监测系统，面向独居老人、夜间照护、养老机构巡护和睡眠健康辅助观察场景，提供生命体征监测、呼噜识别、疑似呼吸暂停提示、睡眠环境评估和紧急求助告警能力。")
    add_para(doc, "系统以 PSOC Edge / Edgi Talk 开发板为核心边缘终端，运行 RT-Thread，利用麦克风、IMU、AHT20 温湿度传感器和毫米波雷达构成多源感知。边缘侧完成小智语音交互、呼噜 int8 模型推理、跌落触发、环境上报和本地 SOS 显示；后端通过 FastAPI 汇聚真实开发板数据，结合雷达呼吸率和呼噜强度进行疑似睡眠呼吸暂停融合判断；前端提供睡眠看护驾驶舱、看护预警中心和睡眠环境分析页面；Android 监护人 App 通过前台服务轮询后端，在锁屏或后台状态下接收紧急通知。")
    ...
    add_bullets(doc, [
        ...
        "IMU 跌落/打翻求助：当板子被打翻或检测到跌落动作时，系统模拟用户无法呼救场景并进入紧急状态。",
    ...
    add_table(doc, ["数据流", "采集端", "后端处理", "用户侧呈现"], [
        ...
        ["紧急事件流", "小智语音关键词 / IMU 跌落", "emergency_voice critical 事件与 active 状态", "开发板 SOS、Web 置顶告警、Android 通知"],
        ["环境感知流", "AHT20 温湿度与环境声", "舒适度状态、环境评分与建议", "环境分析页温湿度折线图"],
    ], widths=[1.15, 1.65, 2.0, 1.85])
    add_para(doc, "上述四条数据流并非孤立展示，而是在后端统一时间轴中汇合。雷达负责“人是否在床、呼吸是否稳定”，麦克风负责“是否出现呼噜或求助语音”，IMU 负责“是否存在设备被打翻/跌落的紧急触发”，环境传感器负责“睡眠环境是否适宜”。多源数据结合后，系统能把普通异常、强告警和设备状态区分开，避免单一传感器误差导致前端刷屏。")
    ...
    add_table(doc, ["模块", "硬件/资源", "接口/协议", "在本项目中的作用"], [
        ...
        ["惯性检测", "IMU", "I2C", "检测开发板跌落/打翻，触发无法呼救场景下的 SOS"],
    ...
    add_para(doc, "从信号路径看，PDM 麦克风的 PCM 数据首先进入共享采集中心，再分别送给唤醒词、小智语音上传和呼噜检测模型；IMU 跌落触发不依赖语音，直接进入 SOS 状态机；AHT20 温湿度样本通过环境任务封装为心跳数据。M55 根据这些输入刷新屏幕状态，在紧急事件中播放报警声并显示“解除紧急状态”按钮，同时通过 Wi-Fi 向后端创建或解除事件。")
    add_figure(doc, "edge_core_pdm_crop.png", "图8-2 Edgi Talk PDM 麦克风相关原理图，用于小智语音和呼噜检测输入（来源：docs/psoc_edge_corev0_2.pdf）", width=5.8)
    add_figure(doc, "edge_basic_imu_aht20_tight.png", "图8-3 Edgi Talk AHT20 与 IMU 相关原理图，支撑环境监测和设备跌落触发（来源：docs/psoc_edge_basic_V0.2.pdf）", width=5.8)
    ...
    add_table(doc, ["任务/模块", "主要职责", "关键输出", "异常处理"], [
        ...
        ["IMU 跌落任务", "检测低重力/撞击/打翻动作", "设备跌落 SOS", "触发后冷却，避免连续误报"],
    ...
    add_para(doc, "Android App 不依赖 BLE 近场连接，而是绑定真实后端地址并通过前台服务持续轮询。这样监护人只要能访问后端，就可以在手机后台或锁屏状态下接收语音 SOS、IMU 跌落和疑似呼吸暂停告警。App 内提供最近报警列表、详情和已处理按钮。")
