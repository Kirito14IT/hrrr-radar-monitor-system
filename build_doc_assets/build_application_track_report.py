...
def add_expanded_design_sections(doc):
    ...
    add_para(doc, "本作品的设计对象不是单一传感器演示，而是夜间看护场景中的完整工作流。夜间睡眠具有低交互、低照明、用户反应慢和异常发生突然等特点，因此系统必须在用户不主动操作的情况下持续观察关键状态；当出现求助、摇晃或疑似呼吸暂停时，又必须把复杂数据转化为看护人员能立即理解的告警。基于这一场景，系统把需求拆分为四类：第一类是生命体征与在床状态，主要由毫米波雷达提供；第二类是声音线索，包括呼噜强度、环境分贝和语音求助，主要由 Edgi Talk M55 的麦克风提供；第三类是环境舒适度，由 AHT20 温湿度和声级趋势共同衡量；第四类是紧急处理闭环，包括本地报警、Web 置顶、手机通知和手动解除。")
    ...
    add_para(doc, "Edgi Talk M55 固件是本作品交互和边缘 AI 的核心。固件在 RT-Thread 下拆分为 Wi-Fi 管理、后端配置、共享音频采集、呼噜检测、小智语音、IMU 摇晃检测、环境上报、屏幕 UI、报警播放和闹钟等任务。Wi-Fi 管理负责首次 AP 配网、配置保存和重启后自动连接；后端配置保存电脑 IP 与端口，当前真实环境统一指向 192.168.0.102:8081；屏幕 UI 负责主页、呼噜守护状态、SOS 状态、解除按钮和闹钟响铃界面。各任务通过轻量状态变量和互斥保护协同，尽量避免在 UI 回调中执行耗时网络请求或音频操作。")
    ...
    add_para(doc, "SOS 状态机包含语音触发和 IMU 触发两条入口。语音入口在小智 STT 文本分支中进行本地关键词匹配，命中“救命、帮帮我、需要帮助、喘不过气、胸口痛、摔倒了”等短语后立即显示 SOS 页面并上报 /emergency。IMU 入口用于模拟病人无法呼救时把床旁开发板打翻到地上的场景，检测到摇晃或异常姿态后同样进入 SOS。两条入口共享本地报警、红色紧急界面、解除按钮和后端事件格式，解除时通过 /emergency/resolve 同步到后端，使前端和 App 不会继续显示 active emergency。")
    ...
    add_para(doc, "全链路测试重点覆盖三类场景。第一类是主动求助：用户说“救命”或“需要帮助”，M55 屏幕进入 SOS，本地报警响起，后端生成 emergency_voice critical 事件，前端预警中心置顶，Android 手机 5 秒内通知，点击开发板或 App 解除后前端同步恢复。第二类是无法呼救：轻微打翻开发板或执行 imu_fall_test，系统以设备摇晃为原因触发 SOS，并显示同样的解除按钮。第三类是疑似呼吸暂停：在雷达低呼吸窗口附近出现呼噜增强或声级反弹，后端生成 suspected_apnea，前端和 App 显示辅助风险提示，但不把它称为医学诊断。")
    ...
    add_para(doc, "SOS 状态机采用统一入口和统一出口。入口包括 STT 关键词命中、IMU 摇晃触发以及调试命令；进入状态后，UI 显示红色 SOS 页面、报警声开始播放、后端收到 /emergency 事件、前端和 App 显示 critical 告警。出口包括开发板解除按钮、Web 处理按钮和 App 已处理按钮；解除后本地停止报警，UI 回到主页，后端事件状态改为 resolved。统一状态机减少了“语音 SOS 能解除但 IMU SOS 不能解除”这类不一致问题。")
...
def build():
    ...
    add_para(doc, "本作品面向夜间睡眠看护与居家安全场景，设计并实现了一套多模态非接触式睡眠监测系统。系统以 PSOC Edge / Edgi Talk 开发板为边缘智能终端，结合毫米波雷达、PDM 麦克风、IMU、AHT20 温湿度传感器和 AIROC Wi-Fi，实现生命体征监测、呼噜识别、疑似睡眠呼吸暂停提示、睡眠环境评分、语音紧急求助和设备摇晃告警。与摄像头看护相比，系统不采集图像，降低隐私压力；与可穿戴设备相比，床旁非接触式方案减少了佩戴、充电和摘除带来的使用门槛。", first_line=True)
    ...
    add_para(doc, "系统围绕“睡眠过程少打扰、异常情况早发现、看护人员可处理”三个目标设计。核心功能包括：毫米波雷达非接触测量心率、呼吸率、距离和在床状态；边缘端呼噜检测模型持续识别环境中的呼噜声；后端融合雷达低呼吸窗口和呼噜变化，提示疑似睡眠呼吸暂停；小智语音识别“救命、需要帮助、喘不过气、摔倒了”等紧急话术并触发 SOS；IMU 检测开发板被打翻或摇晃时也进入紧急状态；AHT20 提供温湿度，配合环境声音分贝形成睡眠环境评分。")
    add_bullets(doc, [
    ...
    add_para(doc, "作品适用于居家养老、独居老人夜间看护、养老机构巡护、慢病人群睡眠观察和宿舍/家庭睡眠环境辅助评估等场景。系统不依赖摄像头，不要求用户长期佩戴设备，适合放置在床旁进行低打扰监测。当用户无法主动操作手机或呼叫看护人员时，可通过语音求助或设备摇晃触发紧急提示。对于长期睡眠观察，系统可以提供呼吸、心率、呼噜、温湿度和事件流的综合记录，为看护人员调整睡眠环境和观察异常趋势提供参考。")
    ...
    add_bullets(doc, [
        ...
        "多传感器融合：雷达负责呼吸/距离，麦克风负责呼噜/求助语音，IMU 负责摇晃触发，AHT20 负责环境评分。",
    ...
    add_para(doc, "系统硬件由两块主要设备构成。第一块是 Edgi Talk / PSoC Edge E84 床旁智能终端，同一芯片内包含 Cortex-M55 主处理核和 Cortex-M33 协处理核：M55 负责共享音频采集、小智语音、呼噜 INT8 推理、IMU 摇晃判断、LVGL 界面、SOS 状态机和网络上报；M33 通过 I2C 周期采集 AHT20 温湿度，并经共享内存把温度、湿度、有效状态、序号和时间戳传递给 M55。第二块是 CY8CKIT-062S2-AI 毫米波雷达板，PSoC 6 通过 25 MHz SPI 与中断引脚采集 BGT60TR13C 雷达帧，通过 I2C 读取 BMI270，并使用 AIROC Wi-Fi/UDP 与本地后端交换雷达帧和启停控制。")
    ...
    add_table(doc, ["连接对象", "硬件接口/协议", "数据内容", "用途"], [
        ...
        ["LSM6DS3 → M55", "I2C0/1，0x6A/0x6B", "三轴加速度、自由落体状态", "设备摇晃与无法呼救场景 SOS"],
    ...
    add_para(doc, "IMU 任务轮询 I2C0/i2c1 上 0x6A、0x6B 地址的 LSM6DS3 系列器件，结合自由落体状态位和加速度幅值判断设备摇晃；一旦触发，统一进入 SOS 状态机，显示解除按钮、播放本地报警并上报 critical 事件。UI、报警、闹钟和网络请求均通过状态变量与任务边界解耦，避免触摸回调中直接删除线程或执行长时间阻塞操作。")
    ...
    add_para(doc, "电路成果主要体现在多开发板资源复用和接口打通：PDM 麦克风输入支撑语音与呼噜检测，IMU 支撑摇晃 SOS，AHT20 通过 I2C 提供温湿度，AIROC Wi-Fi 支撑 HTTP/UDP 通信，雷达模块提供非接触生命体征输入。")
    ...
    add_table(doc, ["测试项", "测试方法", "实现结果"], [
        ...
        ["IMU SOS", "串口命令或低高度软垫打翻测试", "触发设备摇晃紧急事件，并显示手动解除按钮"],
    ...
    add_para(doc, "本作品完成了从边缘硬件、后端融合、前端展示到移动端提醒的完整睡眠看护闭环。项目的核心价值不在于单一传感器精度，而在于多模态数据协同：雷达用于非接触生命体征，麦克风用于呼噜和求助语音，IMU 用于无法呼救时的设备摇晃触发，AHT20 用于环境舒适度评估。通过后端统一状态层，系统把这些输入转化为看护人员容易理解和处理的事件流。")
    ...
    add_para(doc, "系统设计也需要在“强告警”和“误报控制”之间保持谨慎。语音 SOS 和 IMU 摇晃属于高风险事件，系统立即进入 critical；疑似呼吸暂停则采用融合规则，不把单点雷达异常或单独呼噜直接当作呼吸暂停。这种设计更符合看护辅助系统的定位：尽早提醒监护人关注，但不替代医学诊断。通过本项目，我们完成了边缘 AI、RT-Thread 多任务、真实硬件接口、Web 可视化和 Android 后台服务的综合实践，也更理解了从课程样机走向实际应用所需要的工程细节。")
