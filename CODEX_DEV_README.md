...
- [Edgi_Talk_M55_XiaoZhi/applications/imu_fall_monitor.c](Edgi_Talk_M55_XiaoZhi/applications/imu_fall_monitor.c) / [.h](Edgi_Talk_M55_XiaoZhi/applications/imu_fall_monitor.h) — **新模块**:LSM6DS3 自由落体检测,经 I2C0 (地址 0x6A/0x6B,见 [imu_fall_monitor.c:10-12](Edgi_Talk_M55_XiaoZhi/applications/imu_fall_monitor.c#L10));20 ms 轮询;软件阈值 0.35 g (`FALL_THRESHOLD_RAW=5734`) + 硬件 `WAKE_UP_SRC.FF_IA` 双源;60 s 冷却;命中时调 `xz_trigger_emergency_event("xiaozhi_imu_board", "设备自由落体", "...")`。
...
- [Edgi_Talk_M55_XiaoZhi/applications/xiaozhi/xiaozhi.cpp](Edgi_Talk_M55_XiaoZhi/applications/xiaozhi/xiaozhi.cpp) — 小智状态机 + WebSocket + `xz_voice_suspend/resume` (mic0 共享协调);`Message_handle` STT 分支识别 "打开打鼾检测"。
- [Edgi_Talk_M55_XiaoZhi/applications/xiaozhi/xiaozhi.h](Edgi_Talk_M55_XiaoZhi/applications/xiaozhi/xiaozhi.h) — `DeviceState` 枚举 + 对外 API。
...
- 编译时默认: `ENV_BACKEND_TARGET_IP` / `ENV_BACKEND_TARGET_PORT` 宏 (无则 `192.168.0.101:8081`)
...
`STT` 分支命中 ([xiaozhi.cpp:1797](Edgi_Talk_M55_XiaoZhi/applications/xiaozhi/xiaozhi.cpp#L1797)): 调 `xz_find_emergency_phrase(text)` → `xz_trigger_emergency_event("xiaozhi_voice_board", phrase, text)` → 切到 s_emergency_screen;前端在 [alert_center.vue](frontend/src/views/alert_center.vue) 顶部出 SOS 卡 (行 35-78 `v-if="latestEmergencyEvent"`),用户点 "确认已处理" (行 517) → `POST /emergency/resolve` → 屏回主屏。
...
#### 7.5.3 唤醒词 vs 鼾声检测 — **重要区分**

| 模块 | 文件 | 干什么 |
|---|---|---|
| **唤醒词** | [xiaozhi_wakeword.cpp](Edgi_Talk_M55_XiaoZhi/applications/xiaozhi/wake_word/xiaozhi_wakeword.cpp) | Edge Impulse 关键词 `"xiaorui"`,阈值 0.80,冷却 1 s,触发回调 → 进入聊天管道 |
...
> **唤醒词 ≠ 鼾声触发源**。唤醒词只是把设备从 idle 拉进 listening(聊天);鼾声会话**不**被唤醒词自动启动,只能由 (a) LVGL 屏上的 "Snore detect" 按钮 `snore_btn_event_cb()`(xiaozhi_ui.c:319),(b) 语音命令 "打开打鼾检测"(`Message_handle` STT 分支, xiaozhi.cpp:1625),二选一启动。
>
> 两者**共用 `mic0`**,所以启动鼾声前必须 `xz_voice_suspend()`(xiaozhi.cpp:1195 关 mic0 + 唤醒线程),结束鼾声时 `xz_voice_resume()`(xiaozhi.cpp:1208 恢复)。
>
...
| 上行音频 | `audio_send_thread_entry` ([snore_detect.cpp:564](Edgi_Talk_M55_XiaoZhi/applications/xiaozhi/wake_word/snore_detect.cpp#L564)) | 10 s | 累计 10 s mono 16 kHz PCM → TCP socket `POST <backend>:<port>/audio`,`Content-Type: audio/wav`;**默认目标 `192.168.0.101:8081`** (与 env_monitor 共用 backend_target) |
...
| 屏 | 控件指针 (行号) | 进入 |
|---|---|---|
| `s_main_screen` (800×512) | [xiaozhi_ui.c:145](Edgi_Talk_M55_XiaoZhi/applications/xiaozhi/ui/xiaozhi_ui.c#L145) | header flex 行 (状态 60% + 电池) / 虚拟 3D emoji / 底 text (输出 + info) / 中下 "Snore detect" 200×56 按钮 | idle 默认 |
| `s_snore_screen` | [xiaozhi_ui.c:146](Edgi_Talk_M55_XiaoZhi/applications/xiaozhi/ui/xiaozhi_ui.c#L146) | 标题 + 像素艺术"睡觉人"画布 (`snore_draw_sleep_pixelart`, 6 像素 scale, 蓝/青调) + result/score 标签 + "Back" 按钮 | 鼾声按钮 / "打开打鼾检测" 语音 |
...
| `FALL_THRESHOLD_RAW` | `5734` | ~0.35 g (LSB),低于此值判"低重力" |
...
- **双源判定**: `WAKE_UP_SRC.FF_IA=0x20` 置位 **或** `low_g_samples ≥ 4` (软件 80 ms 持续低重力)
...
- 唤醒词 "xiaorui" 命中 → `xz_wakeword_detected_callback` → 进入 chat listening
- 鼾声会话**只**由 (a) LVGL "Snore detect" 按钮 (`snore_btn_event_cb`, xiaozhi_ui.c:319),(b) 语音命令 "打开打鼾检测" (xiaozhi.cpp:1625) 启动
- 共用 `mic0`,所以启鼾声前要 `xz_voice_suspend` 关唤醒线程,结束时 `xz_voice_resume` 恢复

#### 7.6.6 紧急词 **不**走 WebSocket 通道

- 紧急词命中 (`xz_find_emergency_phrase` 命中"救命"等 10 个中文关键词) 后**不**生成 WebSocket 消息,**直接**本地 `xz_trigger_emergency_alarm()` 播 SOS 音 + `xz_post_json_to_backend("/emergency", ...)` 走 RT-Thread WebClient → 后端
- `xz_voice_suspend` **不**调用——紧急播报期间 mic 仍开,但 xiaozhi 的 chat 管道被打断 (状态机切到 `kDeviceStateFatalError`/`Emergency`)
...
输出 `mtb-example-wifi-udp-client.{elf,hex,bin}` (ModusToolbox, BSP=`TARGET=APP_CY8CKIT-062S2-AI`, BGT60TR13C shield)。UDP 目标 `8.133.22.44:9988` 硬编码在 [udp_server.c:152](radar_wifi/source/udp_server.c#L152) — 改成 PC IP。
...
[T+I.x] 设备突然失重 (被打翻 / 老人意外)
  ...
  → 双源判定: `WAKE_UP_SRC.FF_IA=0x20` 硬件中断 **或** `low_g_samples ≥ 4` 软件 80 ms 低重力 ([imu_fall_monitor.c:198-202](Edgi_Talk_M55_XiaoZhi/applications/imu_fall_monitor.c#L198))
...
15. **唤醒词 ≠ 鼾声触发**: 唤醒词 "xiaorui" 命中只进 chat listening。鼾声会话**只**由 LVGL 按钮或 "打开打鼾检测" 语音命令启动;两者共用 `mic0`,启鼾声前 `xz_voice_suspend` 关唤醒。
...
18. **`radar_wifi/source/udp_server.c` 目标 IP 硬编码 `8.133.22.44:9988`**: 是运营商 IP,不是 PC IP!烧板前**必须**改成 PC 实际 IP 并重烧。仓库里那 `udp_server.py` 是 upstream mtb-example 的 LED toggle 脚本,跟雷达固件无关。
...
27. **紧急词命中不通过 WebSocket 走**: `xz_find_emergency_phrase` 命中后**直接**本地播 SOS 音 + `xz_post_json_to_backend("/emergency", ...)` 走 WebClient → 后端;**不**经过 xiaozhi 云 (`api.tenclass.net`) 的 STT 通道。聊天功能正常时,小智云 STT 也会把同一句话转成文本给 xiaozhi.cpp 处理——所以一条"救命"可能**同时**触发: (1) 小智云聊天回复 (2) 紧急 SOS。**这是设计上的双通道,不是 bug**。
