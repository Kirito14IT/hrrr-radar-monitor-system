...
static rt_err_t ST7102_read_regs(struct rt_i2c_client *dev, rt_uint16_t reg,
                                 rt_uint8_t *data, rt_uint8_t len)
{
    struct rt_i2c_msg msgs[2];
    rt_uint8_t reg_buf[ST7102_REGITER_LEN];

    reg_buf[0] = (rt_uint8_t)(reg >> 8);
    reg_buf[1] = (rt_uint8_t)(reg & 0xff);
...
static rt_size_t ST7102_read_point(struct rt_touch_device *touch, void *buf, rt_size_t read_num)
{
    ...
    if (ST7102_read_regs(&ST7102_client, ST7102_Read_Start_Position,
                         read_buf, 8 * ST7102_MAX_TOUCH) != RT_EOK)
    {
