...
5. 联网后设备默认进入 **“守护模式”**，开始呼噜和求助关键词监测：

   ![alt text](figures/8.png)

> **提示**：点击屏幕“对话模式”或按一下开发板上的 **第一个用户按键**，会停止守护监测并立即进入小智语音输入。

## 守护模式与对话模式

- **守护模式（默认）**：本地呼噜模型持续运行；联网时使用小智云持续转写并只匹配 10 个求助词。普通谈话不会播放大模型回复，断网时呼噜检测仍继续。
- **对话模式**：停止呼噜与求助词监测，切换后立即开始聆听，并保留小智原有多轮聊天、表情和语音回复。
- 主界面“守护模式 / 对话模式”双按钮用于显式切换。守护详情页返回主界面不会停止守护。
...
* 烧录完成后，设备上电即可运行示例工程。
* 上电默认进入守护模式；**按一下**顶部按键可切换到对话模式并进入 **聆听中**。
