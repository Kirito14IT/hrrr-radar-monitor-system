...
默认电脑后端地址：`192.168.0.101:8081`。开发板、前端和安卓手机都应连接到这个地址。
...
- [ ] `backend_cfg_status` 显示目标为 `192.168.0.101:8081`
- [ ] 如有旧 IP，执行 `backend_cfg_set 192.168.0.101 8081`
- [ ] Wi-Fi 配置可保存，重启后自动连接
...
- [ ] APK 可安装到安卓手机
- [ ] 首次打开可填写 `http://192.168.0.101:8081`
- [ ] 点击开始监听后显示“监听中”
- [ ] 后端离线时 App 显示离线但不崩溃
- [ ] 触发“小智救命”后 5 秒内收到手机通知
- [ ] 触发 IMU 摇晃后 5 秒内收到手机通知
- [ ] 触发疑似呼吸暂停后收到手机通知
- [ ] 同一个事件不会反复弹重复通知
